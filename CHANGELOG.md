...
- **The six compound tools published an empty input schema.** Their `z.union`
  of per-mode objects serialized to `{"type":"object","properties":{}}` — the
  SDK only converts object schemas and raw shapes to JSON Schema, and a union
  is neither — so an Apps host had no fields to render a form. The compound
  layer now registers a flat shape: the `mode` enum plus every field each mode
  accepts, unioned where a name is shared across modes (e.g. `species` is a
  string in `lookup`/`species` and an array in `lookup`/`sprites`), with the
  per-mode object re-enforcing the real required-field contract inside the
  handler. The smoke suite pins that every compound tool publishes its `mode`
  enum and fields.
...
- **`optimize_team` pair recommendations split the first reason into single
  characters.** The two-slot path spread a string (`...condition ? … : …`) where
  it meant to place one reason element, so `reasons[0]` became one character per
  entry and the output rendered a letter per line. The stray spread is removed;
  the smoke suite now asserts every reason is a multi-character string so the
...
- **The MCP App workspace.** The server now exposes one portable MCP App —
  a single bundled HTML resource registered at `ui://getcompetitive/workspace`
  (`text/html;profile=mcp-app`) and linked from the three App tools. It renders
  three Views over the existing deterministic engine: **Team Doctor**
  (`analyze_team` synergy/diagnose — six sets, labelled heuristic score,
  stacked weaknesses, coverage, speed and top fixes), **Team Builder**
  (`optimize_team` — select a proposed member, validate with `team_io`
  legality, export the paste), and **Matchup Board** (`prepare_matchup` —
  bring four, speed races, calculated damage and heuristic leads, with the
  opponent's set assumptions shown beside each roll). The bundle is built by
  `scripts/build-app.mjs` with esbuild and embedded in the compiled server JS,
  so the stdio, HTTP and serverless Worker entrypoints serve byte-identical
  markup with no runtime filesystem read, no external CDN, fonts or scripts.
- **A pinned App SDK.** `@modelcontextprotocol/ext-apps@1.7.5` (the release
  line that pairs with this server's `@modelcontextprotocol/sdk@^1.30.0`) is
  used for resource registration and App-side messages; the App itself imports
  the SDK's `app-with-deps` bundle. Host support is negotiated through the
  extension capability where the SDK requires it, and ordinary text output is
  preserved unchanged for hosts without Apps support.

### Changed

- **Breaking: the three App tools wrap `structuredContent` in a versioned
  envelope.** `analyze_team`, `optimize_team` and `prepare_matchup` now return
  `{ schemaVersion: 1, view, tool, mode, regulation, data }` in
  `structuredContent`, where `data` is the unchanged analytics payload at the
  requested `detail` level. The model-facing text (the compact JSON summary) is
  byte-for-byte the same; only `structuredContent` consumers migrate their
  reads to `…structuredContent.data.<field>`. `schemaVersion` is the migration
  hook: a View that does not recognise the number renders an unsupported-schema
  state instead of guessing. Every other tool's `structuredContent` is
  untouched, and no tool's analytics field was renamed or reshaped.
- **The evidence app and the App are now distinct surfaces.** The HTTP
  entrypoint's `/` page (the model-converses browser page) is unchanged; the
  MCP App is the sandboxed iframe served through `resources/read`.

### Not included

- App-only tools (`_meta.ui.visibility: ["app"]`) are deferred: every View
  interaction uses the existing public tools, so the model catalog stays at
  nine tools and app-initiated drilldowns request `detail: "evidence"` without
  adding to the model's history. Private endpoints (e.g. `ui_get_team_detail`)
  will only be added if a focused drilldown needs one.

## [7.0.0] - 2026-09-21

### Added

- **Generated sets have a home of their own.** `record_set` files a set the
  reasoning produced — solved from battle observations (`inferred`) or proposed
  for a team (`proposed`) — into a local, per-user record
  (`$GETCOMPETITIVE_STORE`, else `~/.getcompetitive/sets.jsonl`): append-only
  JSON Lines, keyed by a hash of the set itself, so re-recording an identical
  set is a no-op and a changed spread, item or move set becomes a new record.
  `analyze_meta` with `mode: "set"` reads them back only with
  `includeRecorded`, under `recorded`, beside the usage-derived set rather than
  merged into it: a record carries no rank, no usage share and no sample, so
  measured usage stays the single source of the meta and no consumer can
  mistake a generated set for the metagame. `src/threats.data.ts` is untouched
  and still generated by `scripts/build-threats.mjs` alone. `/team-doctor`,
  `/build-around` and `/scout-opponent` now record the sets they change, derive
  or infer — which is what makes the record fill itself.

### Removed

- **Breaking: every lookup payload is Champions-only.** The `lookup` modes no
  longer report anything about the games that came before. `gen` is gone from
  species, moves, items, abilities, natures and types; `isNonstandard`
  ("Past"/"Future"/"Unobtainable"/"CAP") from species, moves, items, abilities
  and types; so are `cannotDynamax`, `canGigantamax`, `isPrimal`,
  `unreleasedHidden`, and the Z-Move and Max Move blocks (`isZ`, `zMove`,
  `isMax`, `maxMove`, `naturalGift`) on moves and items. Learnset event
  distributions drop `generation`, `source` and `emeraldEventEgg`. `calculate`
  drops the `isDynamaxed` set input, and the canonical set shape behind
  `team_io`, `analyze_team` and `record_set` drops `teraType` — a `Tera Type:`
  line in an imported paste is now ignored the way `Shiny:` and `OT:` already
  were. Availability was never this server's to answer in another game's terms
  (`check_legality` answers what a regulation allows), so nothing that remains
  says which game an entry came from or whether it is still obtainable
  elsewhere. `test/champions.mjs` now scans every shaped entry in the dataset
  for the banned fields and for generation markers in its text, so one cannot
  come back unnoticed.
- **Breaking: two acquisition labels in `get_learnset`.** The `movesBySource`
  keys "Virtual Console transfer" and "Dream World" named routes belonging to
  earlier games' distribution machinery; they read "Transfer" and "Other" now.
  Membership is unchanged — every move still appears under exactly the buckets
  it did.

### Changed

- **Retired effect text no longer carries its generation marker.** Showdown
  prefixes the text of an entry it has retired with the game whose rules it
  describes ("(Gen 2) …"); `get_item` strips that prefix, because which earlier
  game a relic came from is not something this server reports.
- **Nine compound tools.** `record_set` joins the surface, so the README,
  `initialize` instructions and those three prompts reflect it.
  `record_set` is the one entrypoint that writes and is annotated as such
  (`WRITE_ANNOTATIONS`; the other eight stay `READ_ONLY_ANNOTATIONS`). It
  resolves its set against the dataset with the same rules as the rest of the
  server — an unknown species, item, nature or an illegal spread is a rejected
  write — and on a runtime with no writable filesystem (the hosted worker) it
  returns an isError naming the path it could not write.
